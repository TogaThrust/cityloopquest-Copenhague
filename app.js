...
function clqStorageKey(suffix) {
  const prefix = (window.CLQ_CITY && window.CLQ_CITY.storagePrefix) || 'copenhague';
...
function getAudioHeardIndices() {
    try {
        const saved = localStorage.getItem('copenhague_audio_heard_indices');
        if (saved) {
            const parsed = JSON.parse(saved);
            if (Array.isArray(parsed)) return parsed.map(Number).filter(n => !Number.isNaN(n));
        }
    } catch (_) { /* ignore */ }
    const legacy = localStorage.getItem('copenhague_audio_clicked_index');
...
function getAudioCompletedIndices() {
    try {
        const saved = localStorage.getItem('copenhague_audio_completed_indices');
        if (saved) {
            const parsed = JSON.parse(saved);
...
function markAudioCompletedForIndex(idx) {
    ...
    localStorage.setItem(
        'copenhague_audio_completed_indices',
        JSON.stringify([...set])
    );
}
...
function markAudioHeardForIndex(idx) {
    ...
    const arr = [...set];
    try {
        localStorage.setItem('copenhague_audio_heard_indices', JSON.stringify(arr));
        localStorage.setItem('copenhague_audio_clicked_index', String(idx));
...
let completedQuizQuestions = (() => {
    const saved = localStorage.getItem("copenhague_completedQuizQuestions");
...
const MONS_LAST_KNOWN_POSITION_KEY = 'copenhague_lastKnownPosition';
...
function resetTourProgressForNewCircuit() {
  currentIndex = 0;
  localStorage.setItem('copenhague_currentIndex', '0');
  localStorage.removeItem('tourCompleted');
  localStorage.removeItem('copenhague_audio_heard_indices');
  localStorage.removeItem('copenhague_audio_clicked_index');
  localStorage.removeItem('copenhague_audio_completed_indices');
  localStorage.removeItem('copenhague_quizEnabled');
  localStorage.removeItem('copenhague_completedQuizQuestions');
  quizEnabled = false;
  quizResumeAwaitingAudio = false;
  quizPendingAdvanceCallback = null;
...
/** Variante fichier image Copenhague : espaces   underscores, casse conservée. */
...
function snapshotTourStateForExternalPage() {
    localStorage.setItem('copenhague_currentIndex', String(currentIndex));
    localStorage.setItem('copenhague_score', String(score));
    try {
...
function advanceToNextPoint() {
    ...
    if (currentIndex === filteredLocations.length - 2) {
        ...
        const visitedPoints = JSON.parse(localStorage.getItem('copenhague_visitedPoints') || '[]');
        if (!visitedPoints.includes(currentIndex)) {
            visitedPoints.push(currentIndex);
            localStorage.setItem('copenhague_visitedPoints', JSON.stringify(visitedPoints));
        }
        ...
        steps = [];
        currentStepIndex = 0;
        localStorage.setItem("copenhague_currentIndex", currentIndex);
        localStorage.setItem("copenhague_score", score);
...
function clearQuizCompletionFromIndex(fromIdx) {
  ...
  localStorage.setItem('copenhague_completedQuizQuestions', JSON.stringify(completedQuizQuestions));
}
...
function updateQuizFooterButton() {
  ...
  const show = !guidanceMode && localStorage.getItem('copenhague_quizEnabled') === 'false';
...
function setQuizEnabledRuntime(enabled) {
  quizEnabled = enabled;
  localStorage.setItem('copenhague_quizEnabled', enabled ? 'true' : 'false');
  if (enabled) localStorage.removeItem('copenhague_quizStopped');
  updateQuizFooterButton();
...
function stopQuizPermanently() {
  ...
  localStorage.setItem('copenhague_quizStopped', 'true');
...
function showQuizPrompt(callback) {
  if (localStorage.getItem("copenhague_quizEnabled") !== null) {
    callback(localStorage.getItem("copenhague_quizEnabled") === "true");
    return;
  }
  const overlay = document.createElement('div');
  ...
  btnYes.style.cursor = 'pointer';
  btnYes.addEventListener('click', () => {
    localStorage.setItem("copenhague_quizEnabled", "true");
    quizEnabled = true;
    completedQuizQuestions = {};
    localStorage.removeItem('copenhague_completedQuizQuestions');
    localStorage.removeItem('copenhague_quizStopped');
    document.body.removeChild(overlay);
    updateQuizFooterButton();
    callback(true);
  });
  ...
  btnNo.style.cursor = 'pointer';
  btnNo.addEventListener('click', () => {
    localStorage.setItem("copenhague_quizEnabled", "false");
    quizEnabled = false;
    document.body.removeChild(overlay);
    updateQuizFooterButton();
    callback(false);
...
function initializeMainLogic() {
    ...
    const savedCurrentIndex = localStorage.getItem("copenhague_currentIndex");
    const savedScore = localStorage.getItem("copenhague_score");
    const savedQuizEnabled = freshCircuitStart
        ? null
        : localStorage.getItem("copenhague_quizEnabled");
    const savedCompletedQuizQuestions = freshCircuitStart
        ? null
        : localStorage.getItem("copenhague_completedQuizQuestions");
    ...
    if (forceTarget) {
        ...
        const visitedPoints = safeJsonParse(localStorage.getItem('copenhague_visitedPoints'), []);
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    nextButton.addEventListener('click', () => {
        ...
        if (localStorage.getItem("copenhague_quizEnabled") === null) {
            ...
            quizEnabled = localStorage.getItem("copenhague_quizEnabled") === "true";
            if (quizEnabled) {
                showQuizForPointIndex(leavingIndex, advanceAfterQuiz, { force: true });
            } else {
                advanceAfterQuiz();
